...
--   [FENTI-04a]  Self aura (meshes / particles / RS custom avatar — Obsidian Aura tab)
...
--   [FENTI-16]   Dialogue helpers (IIFE → _G.fentiDialogue.instaDialogNPC)
...
do
    ...
    -- SELF AURA — state
    -- ============================================================================
    local SelfAuraState = { Objects = {}, ChildConn = nil, Character = nil, LuffyCleanup = nil }
    local currentAura = "off"
    
    local function clear_self_aura()
        if SelfAuraState.LuffyCleanup then
            pcall(SelfAuraState.LuffyCleanup)
            SelfAuraState.LuffyCleanup = nil
        end
    ...
    -- Full-body overlay: clone a Model from ReplicatedStorage (import .rbxm in Studio → name it FentiLuffyAvatar).
    local LUFFY_RS_MODEL_NAMES = { "FentiLuffyAvatar", "LuffyAvatar" }
    local function find_custom_avatar_template()
        local rs = game:GetService("ReplicatedStorage")
        for _, name in ipairs(LUFFY_RS_MODEL_NAMES) do
            local m = rs:FindFirstChild(name, true)
    ...
    local function apply_luffy_custom_avatar(character)
        ...
        clone.Name = "SelfAura_LuffyAvatar"
        ...
        track_self_aura_object(clone)
    
        local hidden = {}
        for _, p in ipairs(character:GetDescendants()) do
            if p:IsA("BasePart") and not clone:IsAncestorOf(p) then
                hidden[#hidden + 1] = { p, p.LocalTransparencyModifier }
                p.LocalTransparencyModifier = 1
            end
        end
    
        local rigConn = nil
        local function restore_body()
            if rigConn then
                rigConn:Disconnect()
                rigConn = nil
            end
            for _, row in ipairs(hidden) do
                local p, orig = row[1], row[2]
                if p and p.Parent then
                    p.LocalTransparencyModifier = orig
                end
            end
        end
        SelfAuraState.LuffyCleanup = restore_body
        for _, d in ipairs(clone:GetDescendants()) do
            if d:IsA("BasePart") then
                d.CanCollide = false
                d.Massless = true
        ...
        rigConn = RunService.RenderStepped:Connect(function()
            if not character.Parent or not clone.Parent then return end
            for _, src in ipairs(character:GetDescendants()) do
                if src:IsA("BasePart") and not clone:IsAncestorOf(src) then
                    local tgt = clone:FindFirstChild(src.Name, true)
                    if tgt and tgt:IsA("BasePart") then
                        tgt.CFrame = src.CFrame
                    end
                end
            end
        end)
    ...
    local function apply_self_aura(character)
        ...
        elseif currentAura == "luffy avatar"    then apply_luffy_custom_avatar(character)
        else
            for _, desc in ipairs(character:GetDescendants()) do
    ...
    -- Primary persistence: executor workspace file (writefile). Options.FishSpotsSaveJSON kept in sync for Config save.
    local FENTI_FISH_SPOTS_DIR = "fenti"
    local FENTI_FISH_SPOTS_FILE = FENTI_FISH_SPOTS_DIR .. "/fishing_spots.json"
    local function fentiFishSpotsToJson()
        local spots = {}
        for _, e in ipairs(fentiCustomFishSpotEntries) do
            local cf = e.cf
            if typeof(cf) == "CFrame" then
                local p, l = cf.Position, cf.LookVector
                table.insert(spots, { name = e.name, x = p.X, y = p.Y, z = p.Z, lx = l.X, ly = l.Y, lz = l.Z })
            end
        end
        local payload = { v = 1, sel = fentiSelectedFishSpotName, spots = spots }
        local ok, s = pcall(function() return HttpService:JSONEncode(payload) end)
        return ok and s or '{"v":1,"sel":"Default (hub)","spots":[]}'
    end
    local function fentiFishSpotsApplyJson(raw)
        if type(raw) ~= "string" or raw == "" then return end
        local ok, data = pcall(function() return HttpService:JSONDecode(raw) end)
        if not ok or type(data) ~= "table" then return end
        while #fentiCustomFishSpotEntries > 0 do table.remove(fentiCustomFishSpotEntries, 1) end
        local list = data.spots
            ...
            if type(row) == "table" and type(row.name) == "string" and row.name ~= "" then
                if row.name ~= "Default (hub)" and row.name ~= "Cliff spot" then
                    ...
                    if px and py and pz then
                        ...
                        local lx, ly, lz = tonumber(row.lx), tonumber(row.ly), tonumber(row.lz)
                        local look = (lx and ly and lz) and Vector3.new(lx, ly, lz) or Vector3.new(0, 0, -1)
                        if look.Magnitude < 1e-4 then look = Vector3.new(0, 0, -1) end
                        table.insert(fentiCustomFishSpotEntries, { name = row.name, cf = CFrame.lookAt(pos, pos + look) })
                    end
                end
            end
        end
        if type(data.sel) == "string" and data.sel ~= "" then
            fentiSelectedFishSpotName = data.sel
        end
    end
    local function fentiFishSpotsPushToSaveOption()
        local json = fentiFishSpotsToJson()
        pcall(function()
            if writefile then
                if makefolder then pcall(function() makefolder(FENTI_FISH_SPOTS_DIR) end) end
                writefile(FENTI_FISH_SPOTS_FILE, json)
            end
        end)
        pcall(function()
            local opt = Options and Options.FishSpotsSaveJSON
            if opt then
                if opt.SetValue then opt:SetValue(json)
                elseif opt.Value ~= nil then opt.Value = json end
            end
        end)
    end
    ...
    local function fentiNormalizeSaintsPartName(partName)
        if type(partName) ~= "string" then return "" end
        local norm = string.lower(partName:gsub("[%s'_%-]", ""))
        if string.sub(norm, 1, 5) == "saint" and string.sub(norm, 6, 6) ~= "s" then
            norm = "saints" .. string.sub(norm, 6)
        end
        return norm
    end
    ...
    local function fireHorsePrompt(horse)
        ...
        for _, desc in pairs(horse:GetDescendants()) do
            if desc:IsA("ProximityPrompt") then
                pcall(function()
                    local px = _G.FentiProximity
                    if px and type(px.safeFireProximityPrompt) == "function" then px.safeFireProximityPrompt(desc) end
                end)
                fired = true
            end
        end
    ...
    local function fentiGetProximityPromptKeyCode(prompt)
        local kc = Enum.KeyCode.E
        if not prompt then return kc end
        pcall(function()
            local k = prompt.KeyboardKeyCode
            if k and k ~= Enum.KeyCode.Unknown then kc = k end
        end)
        return kc
    end

    -- Hold a key down with VirtualInputManager (gameProcessedEvent must stay false — same as QTE shim).
    local function fentiVimHoldKeyCode(keyCode, holdSec)
        if not VIM or not keyCode then return false end
        local function keyDown()
            local ok = pcall(function() VIM:SendKeyEvent(true, keyCode, false, false) end)
            if ok then return true end
            return pcall(function() VIM:SendKeyEvent(true, keyCode, false) end)
        end
        local function keyUp()
            local ok = pcall(function() VIM:SendKeyEvent(false, keyCode, false, false) end)
            if ok then return true end
            return pcall(function() VIM:SendKeyEvent(false, keyCode, false) end)
        end
    ...
    local function fentiRestorePromptProps(prompt, oldHold, oldMax, oldEnabled, oldLos, delaySec)
        task.delay(delaySec or 0.42, function()
            pcall(function()
                if not prompt or not prompt.Parent then return end
                if oldHold ~= nil then prompt.HoldDuration = oldHold end
                if oldMax ~= nil then prompt.MaxActivationDistance = oldMax end
                if oldEnabled ~= nil then prompt.Enabled = oldEnabled end
    ...
    local function fentiFireProximityWithVirtualHold(prompt, opts)
        ...
        pcall(function()
            ...
            prompt.MaxActivationDistance = math.min(FENTI_PROMPT_MAX_STRETCH, math.max(m, 16))
        end)
        local baseHold = 0.5
        ...
        if VIM then
            ...
            usedVim = fentiVimHoldKeyCode(keyCode, holdSec)
    ...
    _G.fentiFireProximityFull = fentiFireProximityFull
    ...
    _G.fentiSpamNearbyChestPrompts = function(radius, rounds, delayBetween)
        radius = radius or 58
        rounds = rounds or 14
        delayBetween = delayBetween or 0.16
        task.spawn(function()
            ...
            for _ = 1, rounds do
                ...
                pcall(function()
                    ...
                    pcall(function() patchNearbyPrompts(root.Position, radius + 26) end)
    ...
    -- Single deferred hold — a second delayed pass restarted InputHold / restore timers and reset progress halfway.
    local function fentiDeferSafeProximityFire(prompt, opts)
    ...
    local function fentiRadiusProximityTick()
        ...
        if Toggles.FentiUnifiedChests and Toggles.FentiUnifiedChests.Value then
            ...
            for _, folder in ipairs(rootsR) do
                for _, box in ipairs(folder:GetChildren()) do
                    if box.Name == "ChestBox" and box.Parent then
                        local part = box:IsA("BasePart") and box or (box.PrimaryPart or box:FindFirstChildWhichIsA("BasePart"))
                        if part and (part.Position - root.Position).Magnitude <= rChestInstant then
                            local pr = box:FindFirstChildWhichIsA("ProximityPrompt", true)
                            if pr then nearChestPrompts[#nearChestPrompts + 1] = pr end
                        end
                    end
                end
            end
            if #nearChestPrompts > 0 then
                local nowT = tick()
                ...
                if type(rawget(_G, "fireproximityprompt")) ~= "function" then
                    local clk = os.clock()
                    local lastT = _G.fentiChestPromptLastFire
                    for _, pr in ipairs(nearChestPrompts) do
                        local last = lastT[pr]
                        if not last or (clk - last) >= FENTI_CHEST_PROMPT_COOLDOWN then
                            lastT[pr] = clk
                            local p = pr
                            task.defer(function()
                                pcall(fentiFireChestProximityInstant, p)
                            end)
                        end
                    end
                end
        ...
        if saintsInstaArm then
            ...
            local function pushModelSaintParts(m)
                if not m or not m:IsA("Model") then return end
                for _, d in ipairs(m:GetDescendants()) do
                    if d:IsA("BasePart") then pushIfSaintPart(d) end
                end
            end
            ...
            if #nearParts > 0 then
                ...
                table.sort(nearParts, function(a, b)
                    return (a.Position - root.Position).Magnitude < (b.Position - root.Position).Magnitude
                end)
                local lastT = _G.fentiSaintsInstaPromptLastFire
                local clk = os.clock()
                local bestPrompt = nil
                for _, ch in ipairs(nearParts) do
                    if saintsClaimLock[ch] then
                    else
                        local prompt = ch:FindFirstChildWhichIsA("ProximityPrompt", true)
                        if prompt then
                            local pt = lastT[prompt]
                            if not pt or (clk - pt) >= FENTI_SAINTS_INSTA_PROMPT_COOLDOWN then
                                bestPrompt = prompt
                                break
                            end
                        end
                ...
                if bestPrompt then
                    lastT[bestPrompt] = clk
                    fentiDeferSafeProximityFire(bestPrompt)
                end
            end
        end
    ...
    -- Without patchNearbyPrompts, chest ProximityPrompts often keep tiny MaxActivationDistance so VIM/assist never sees you in range.
    -- One nearest chest per tick + cooldown (was: defer-safe double-fire + all chests each tick → hold reset mid-bar).
    task.defer(function()
        local radius = 48
        local r2 = radius * radius
        while true do
            local interval = (isRunning and 0.52) or 0.42
            task.wait(interval)
            if _G.AutoCollect then
                pcall(function()
                        ...
                        for _, box in ipairs(chests:GetChildren()) do
                            if box.Parent then
                                ...
                                if part then
                                    ...
                                    if d:Dot(d) <= r2 then nearChest = true; break end
                                end
                            end
                        end
                        if nearChest then break end
                    end
                    if nearChest and instaChest then
                        local now = tick()
                        local nextPatch = rawget(_G, "fentiChestInstaPatchNext") or 0
                        if now >= nextPatch then
                            _G.fentiChestInstaPatchNext = now + 1.1
                            pcall(function() patchNearbyPrompts(root.Position, radius + 26) end)
                        end
                    end
                    local bestPrompt, bestD2 = nil, nil
                    for _, chests in ipairs(rootsChest) do
                        ...
                        for _, box in ipairs(chests:GetChildren()) do
                            if box.Parent then
                                ...
                                if part then
                                    ...
                                    if dist2 <= r2 then
                                        local prompt = box:FindFirstChildWhichIsA("ProximityPrompt", true)
                                        if prompt and (not bestD2 or dist2 < bestD2) then
                                            bestD2, bestPrompt = dist2, prompt
                                        end
                                    end
                                end
                            end
                    ...
                    -- Instant chest toggle: radius tick fires HoldDuration=0 path only (no duplicate virtual hold here).
                    if bestPrompt and not instaChest then
                        local clk = os.clock()
                        local lastT = _G.fentiChestPromptLastFire
                        local last = lastT[bestPrompt]
                        if not last or (clk - last) >= FENTI_CHEST_PROMPT_COOLDOWN then
                            lastT[bestPrompt] = clk
                            local p = bestPrompt
                            task.spawn(function()
    ...
    _G.fentiDisconnectIYChestInstantPP = function()
        if fentiIYChestInstantConn then
            pcall(function() fentiIYChestInstantConn:Disconnect() end)
            fentiIYChestInstantConn = nil
        end
    end
    _G.fentiConnectIYChestInstantPP = function()
        _G.fentiDisconnectIYChestInstantPP()
        local fpp = rawget(_G, "fireproximityprompt")
        if type(fpp) ~= "function" or not ProximityPromptService then return false end
        local ok, conn = pcall(function()
            return ProximityPromptService.PromptButtonHoldBegan:Connect(function(prompt)
                if not prompt or not prompt:IsA("ProximityPrompt") then return end
                if not ichest(prompt) then return end
                pcall(fpp, prompt)
                pcall(fpp, prompt, 0)
            end)
        end)
        if ok and conn then
            fentiIYChestInstantConn = conn
            return true
        end
        return false
    end
    ...
    -- ----------------------------------------------------------------------------
    -- [FENTI-16] 16. DIALOGUE HELPERS (IIFE — own local register pool; avoids chunk limit ~200)
    -- ----------------------------------------------------------------------------
    do
    (function()
    ...
    local function getFishCastDistanceStuds()
        local opt = Options and Options.FishCastDistance
        local v = opt and opt.Value
        if type(v) == "number" and v >= 8 and v <= 220 then return v end
        return 95
    end
    _G.fentiGetFishCastDistanceStuds = getFishCastDistanceStuds
    -- Always cast horizontally in front of camera (fallback HRP): distance from slider, tiny yaw jitter only (no pitch into ground).
    local function getRandomVector(longCastMul)
        ...
        if not playerPos then return Vector3.zero end
        local camera = workspace.CurrentCamera
        ...
        local yaw = (math.random() - 0.5) * math.rad(7)
        local dir = (CFrame.new(Vector3.zero, forward) * CFrame.Angles(0, yaw, 0)).LookVector
        local dist = getFishCastDistanceStuds()
        if longCastMul == true then
            dist = math.min(dist * 1.14, dist + 32, 210)
        end
        local flat = playerPos + dir * dist
        return Vector3.new(flat.X, playerPos.Y + 0.2, flat.Z)
    end
    ...
    -- Cast: bait + 0.3s pause, then horizontal-forward aim at FishCastDistance studs (camera/HRP forward).
    ...
    local function castRodLong()
        ...
        local aim = getRandomVector(true)
    ...
    return {
        ...
        getRandomVector = function() return getRandomVector(false) end,
        getFishCastDistanceStuds = getFishCastDistanceStuds,
        castRod = castRod,
    ...
    (function()
        ...
        local function fireFishingBobberPrompt(fishingPart)
            ...
            if not prompt then return false end
            local px0 = _G.FentiProximity
            if px0 and type(px0.safeFireProximityPrompt) == "function" then
                pcall(function() px0.safeFireProximityPrompt(prompt) end)
                task.wait(0.45)
                return true
            end
        ...
        local function fentiFishingPromptAssistTick(anchorPosition)
            pcall(function()
                ...
                if typeof(castHint) == "Vector3" and castHint.Magnitude > 1 then
                    local cd = (type(_G.fentiGetFishCastDistanceStuds) == "function" and _G.fentiGetFishCastDistanceStuds()) or 95
                    local gate = math.max(58, cd + 32)
                    hintGateSq = gate * gate
                end
                ...
                if bestPr then
                    ...
                    task.defer(function()
                        pcall(function()
                            local px1 = _G.FentiProximity
                            if px1 and type(px1.safeFireProximityPrompt) == "function" then px1.safeFireProximityPrompt(pr)
    ...
    local function findBestFishingBobberPart(playerPos, maxDist, castAimHint, deepScan)
        ...
        if typeof(castAimHint) == "Vector3" and castAimHint.Magnitude > 1 then
            local cd = (type(_G.fentiGetFishCastDistanceStuds) == "function" and _G.fentiGetFishCastDistanceStuds()) or 95
            local gate = math.max(62, cd + 34)
            hintSqGate = gate * gate
            aimReject = math.max(44, cd * 0.78 + 22)
        end
        local function consider(part)
            if not part:IsA("BasePart") then return end
            if aimReject and typeof(castAimHint) == "Vector3" and castAimHint.Magnitude > 1 then
                if (part.Position - castAimHint).Magnitude > aimReject then return end
            end
            local c = part.Color
    ...
    local function findRedBobberPartNear(playerPos, maxDist, castAimHint)
        if not playerPos then return nil end
        maxDist = maxDist or 100
        local maxSq = maxDist * maxDist
        local cd = (type(_G.fentiGetFishCastDistanceStuds) == "function" and _G.fentiGetFishCastDistanceStuds()) or 95
        local aimLim = typeof(castAimHint) == "Vector3" and castAimHint.Magnitude > 1
            and math.max(40, cd * 0.75 + 20)
            or nil
        ...
        for _, p in ipairs(workspace:GetChildren()) do
            if p.Name == "Part" and p:IsA("BasePart") then
                ...
                if c.R > 0.92 and c.G < 0.35 and c.B < 0.35 then
                    ...
                    if dsq <= maxSq then
                        if aimLim then
                            local da = (p.Position - castAimHint).Magnitude
                            if da > aimLim then
                                -- skip reds that cannot be from this cast (old bobber / ground junk)
                            else
                                local score = dsq + da * da * 0.45
                                if score < bestScore then bestScore, best = score, p end
                            end
                        else
                            if dsq < bestScore then bestScore, best = dsq, p end
                        end
                    end
                end
            end
    ...
    _G.fentiFishingLoop = function()
        ...
        while isRunning do
            ...
            local castD = (type(_G.fentiGetFishCastDistanceStuds) == "function" and _G.fentiGetFishCastDistanceStuds()) or 95
            local scanR = math.min(210, math.max(115, castD + 72))
            local fishingPart = findRedBobberPartNear(playerPos, scanR, lastCastAim)
            if not fishingPart then
                fishingPart = findBestFishingBobberPart(playerPos, scanR, lastCastAim, false)
            end
            if not fishingPart then
                task.wait(1.15)
                refreshCharacter()
                playerPos = (humanoidRootPart and humanoidRootPart.Position) or playerPos
                fishingPart = findBestFishingBobberPart(playerPos, scanR, lastCastAim, true)
            end
            if fishingPart and typeof(lastCastAim) == "Vector3" and lastCastAim.Magnitude > 1 then
                local md = (fishingPart.Position - lastCastAim).Magnitude
                local lim = math.max(46, castD * 0.82 + 24)
                if md > lim then
                    pcall(function() _G.fentiFish.fishDbg("loop: bobber rejected (far from cast aim " .. math.floor(md) .. " > " .. math.floor(lim) .. ")") end)
                    fishingPart = nil
                end
            end
            if fishingPart then
                ...
                if _G.fentiWaitForFishBite(fishingPart) then
                    ...
                    if humanoidRootPart then
                        ...
                        local openedNearby = false
                        for _, chestFolder in ipairs(roots) do
                            if openedNearby then break end
                            for _, chest in ipairs(chestFolder:GetChildren()) do
                                if chest.Name == "ChestBox" and chest.Parent then
                                    local chestPos
                                    pcall(function()
                                        if chest:IsA("Model") then chestPos = chest:GetPivot().Position
                                        elseif chest:IsA("BasePart") then chestPos = chest.Position end
                                    end)
                                    if chestPos and (chestPos - humanoidRootPart.Position).Magnitude <= 52 then
                                        local pr = chest:FindFirstChildWhichIsA("ProximityPrompt", true)
                                        if pr then
                                            if _G.fentiFireChestProximityInstant then pcall(_G.fentiFireChestProximityInstant, pr)
                                            elseif _G.fentiFireProximityFull then pcall(_G.fentiFireProximityFull, pr)
                                            elseif _G.fentiFireProximityMinimal then pcall(_G.fentiFireProximityMinimal, pr)
                                            elseif fireproximityprompt then pcall(fireproximityprompt, pr) end
                                            task.wait(0.35)
                                            openedNearby = true
                                            break
                                        end
    ...
    FentiFarm.pickupCorpseAt = function(spawn)
        ...
        local Px = _G.FentiProximity
        local wfp = Px and Px.waitForPrompt
        local spm = Px and Px.spamPrompt
        local fap = Px and Px.fireAllPrompts
    
        local picked = false
        local prompt = wfp and wfp(spawn, 3)
        if prompt and spm then spm(prompt, 8, 0.15); picked = true end
        if not picked and fap then picked = fap(spawn, 3) end
        ...
        for _, obj in pairs(workspace:GetChildren()) do
            if obj:IsA("Model") or obj:IsA("BasePart") then
                ...
                if objPos and humanoidRootPart and (objPos - humanoidRootPart.Position).Magnitude < 25 then
                    local p = wfp and wfp(obj, 1)
                    if p and spm then spm(p, 5, 0.15); picked = true
                    elseif fap and fap(obj, 2) then picked = true end
                end
            end
        end
        local function fireSaintsCorpsePart(part)
            ...
            elseif fap and fap(part, 4) then picked = true end
        end
        local function scanCorpseFolderForSaints(folder)
            if not folder then return end
            for _, ch in pairs(folder:GetChildren()) do
                if ch:IsA("BasePart") and fentiSaintsPartEligible(ch.Name) then
                    fireSaintsCorpsePart(ch)
                elseif ch:IsA("Model") then
                    for _, d in pairs(ch:GetDescendants()) do
                        if d:IsA("BasePart") and fentiSaintsPartEligible(d.Name) then
                            fireSaintsCorpsePart(d)
                        end
                    end
                end
            end
        ...
        scanCorpseFolderForSaints(RS:FindFirstChild("CorpseParts"))
        scanCorpseFolderForSaints(workspace:FindFirstChild("CorpseParts"))
        return picked
    end
    ...
    -- Must match fentiSaintsPartEligible (case-insensitive "saints" substring); old name:find("Saints") missed lowercase / mixed names.
    ...
    local function claimSaintsPart(part)
        ...
        local function fireSaintPromptOnce()
            pcall(function()
                ...
                elseif PxS and type(PxS.safeFireProximityPrompt) == "function" then
                    PxS.safeFireProximityPrompt(p2)
                end
            end)
        end
        ...
        if not claimedOk then
            pcall(function()
                if humanoidRootPart and part.Parent then
                    ...
                    if PxS and PxS.patchAndFirePromptsOnInstance then PxS.patchAndFirePromptsOnInstance(part, "saintsClaim", 0.1) end
                end
            end)
            task.wait(0.15)
            for _round = 1, math.max(1, saintsClaimPromptRounds) do
                if not part:IsDescendantOf(workspace) then break end
                refreshCharacter()
                local pr = PxS and PxS.waitForPrompt and PxS.waitForPrompt(part, 1.8)
                if pr and PxS and PxS.spamPrompt then
                    PxS.spamPrompt(pr, 8, 0.08)
                else
                    pcall(function()
                    ...
                    if PxS and PxS.fireAllPrompts then PxS.fireAllPrompts(part, 3) end
                end
                task.wait(0.22)
                if hasSaintPart(partName) then claimedOk = true break end
            end
    ...
    FentiFarm.startSaintsMonitor = function()
        ...
        local function hookCorpsePartsTree(cp)
            ...
            -- Nested parts (models under CorpseParts) — ChildAdded only sees top-level.
            table.insert(saintsConns, cp.DescendantAdded:Connect(function(inst)
                if not saintsEnabled then return end
                if inst:IsA("BasePart") and matchesSaintsFilter(inst.Name) and not hasSaintPart(inst.Name) then
                    task.defer(function()
    ...
    if (type)(Library.CreateLoading) == "function" then
        ...
        if okL and Ld then
            _G.fentiObsidianLoader = Ld
            pcall(function()
                Ld:SetMessage("Loading hub")
                Ld:SetDescription("Preparing…")
                Ld:SetCurrentStep(1)
                Ld:ShowSidebarPage(true)
                if Ld.Sidebar then
                    Ld.Sidebar:AddLabel("<b>fenti</b> v" .. tostring(FENTI_SCRIPT_VERSION))
                    if player then
                        Ld.Sidebar:AddLabel("Player: <b>" .. player.Name .. "</b>")
                    end
                    Ld.Sidebar:AddDivider()
                    Ld.Sidebar:AddLabel("<b>Updates</b> — " .. FENTI_HUB_CHANGELOG_BLURB, true)
                end
            end)
            task.wait(0.55)
        end
    end
    local _fentiLd = _G.fentiObsidianLoader
    if _fentiLd then
        pcall(function()
            _fentiLd:SetDescription("Creating window…")
        end)
        task.wait(0.5)
    end
    ...
    if not _G.fentiWindow then
        ...
        if _G.fentiObsidianLoader then
            pcall(function()
                ...
                EL:ShowErrorPage(true)
                EL:SetErrorMessage("Could not open the menu.\n\n" .. tostring(_G.fentiCWErr))
                EL:SetErrorButtons({
                    Close = {
                        Title = "Close",
                        Variant = "Primary",
    ...
    if _G.fentiObsidianLoader then
        pcall(function()
            _G.fentiObsidianLoader:SetCurrentStep(2)
            _G.fentiObsidianLoader:SetDescription("Building interface…")
        end)
        task.wait(0.65)
    end
    do
    ...
    InfoLeft.User:AddLabel("<b>Xeno:</b> <font color='#00FF00'>working</font>")
    ...
    InfoLeft.Game:AddDivider()
    InfoLeft.Game:AddButton({ Text = "Copy join command (executor)", Func = function()
        if not setclipboard then Library:Notify("Clipboard not supported.", 3); return end
        ...
        if type(jid) == "string" and jid ~= "" then
            ...
            line = string.format('game:GetService("TeleportService"):Teleport(%s, game.Players.LocalPlayer)', pid)
        end
        setclipboard(line)
        Library:Notify("Join command copied.", 2)
    end })
    ...
    InfoRightOverview:AddLabel("<b>Aura</b> — Client visuals on your character.", true)
    InfoRightOverview:AddLabel("<b>Config</b> — Theme, save, unload.", true)
    ...
    FishingLeft:AddSlider("FishCastDistance", {
        Text = "Cast distance (studs)",
        Default = 95,
        Min = 12,
        Max = 200,
        Rounding = 0,
    ...
    AimbotBox:AddButton({
        ...
        Func = function()
            ...
            task.spawn(function()
                ...
                Library:Notify("External aimbot loaded — check in-game.", 4)
            end)
        end,
    })
    end end -- Aimbot
    ...
    local _fentiFishSpotsSaveInput = MenuGroup:AddInput("FishSpotsSaveJSON", { Text = " ", Default = "", Finished = false })
    pcall(function()
        local el = _fentiFishSpotsSaveInput
        if not el then return end
        local h = el.Holder or el.Container or el.Main
    ...
    do
        local OL = _G.fentiObsidianLoader
        if OL then
            pcall(function()
                OL:SetCurrentStep(3)
                OL:SetDescription("Loading config…")
            end)
            task.wait(0.55)
        end
    end
    ...
    if ThemeManager and SaveManager and Tabs.Config then
        pcall(function()
            ...
            pcall(function()
                local diskRaw = nil
                pcall(function()
                    if isfile and readfile and isfile(FENTI_FISH_SPOTS_FILE) then
                        diskRaw = readfile(FENTI_FISH_SPOTS_FILE)
                    end
                end)
                if type(diskRaw) == "string" and #diskRaw > 4 then
                    fentiFishSpotsApplyJson(diskRaw)
                    pcall(function()
                        local opt = Options and Options.FishSpotsSaveJSON
                        if opt then
                            if opt.SetValue then opt:SetValue(diskRaw)
                            elseif opt.Value ~= nil then opt.Value = diskRaw end
                        end
                    end)
                else
                    local j = Options.FishSpotsSaveJSON and Options.FishSpotsSaveJSON.Value
                    if type(j) == "string" and j ~= "" then
                        fentiFishSpotsApplyJson(j)
                        pcall(function()
                            if writefile then
                                if makefolder then pcall(function() makefolder(FENTI_FISH_SPOTS_DIR) end) end
                                writefile(FENTI_FISH_SPOTS_FILE, j)
                            end
                        end)
                    end
                end
                ...
                local selOk = false
                for _, n in ipairs(vals) do if n == fentiSelectedFishSpotName then selOk = true; break end end
                if not selOk then fentiSelectedFishSpotName = "Default (hub)" end
                if Options.FishSpotPick then
                    Options.FishSpotPick:SetValues(vals)
                    pcall(function() Options.FishSpotPick:SetValue(fentiSelectedFishSpotName) end)
                end
            end)
            pcall(function()
    ...
    if not _fentiHubUiOk then
        ...
        if _G.fentiObsidianLoader then
            pcall(function()
                local EL = _G.fentiObsidianLoader
                EL:ShowErrorPage(true)
                EL:SetErrorMessage("Hub failed to build:\n\n" .. tostring(_fentiHubUiErr))
                EL:SetErrorButtons({
                    Close = {
                        Title = "Close",
                        Variant = "Primary",
